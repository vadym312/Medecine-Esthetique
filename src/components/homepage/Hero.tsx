...
export const Hero: React.FC = () => {
  return (
    <section className="relative h-screen w-full">
      <div className="reltive bg-cover bg-center bg-black/50 inset-0 absolutes">
        <video
          autoPlay
          muted
          playsInline
          loop
          className="w-full h-full absolute object-cover inset-0 z-0"
          src={hero.videoUrl}
        ></video>
        <div className="absoulte inset-0 bg-black/60 z-20" />
      </div>
      <div className="relative h-full flex flex-col items-center justify-center text-center">
        ...
        >
          ...
          <Button className="bg-white hover:bg-white/90 text-black px-8 py-3 rounded-full text-sm transition-colors">
            <Link
              href="https://drelard.booking.nextmotion.net/site-web"
              target="_blank"
            >
              {hero.ctaText}
            </Link>
